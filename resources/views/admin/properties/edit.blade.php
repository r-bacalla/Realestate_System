        ...
        <form action="{{ route('admin.properties.update', $property) }}"
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <div class="space-y-4">
                    ...
                    <input type="text" name="name" value="{{ old('name', $property->name) }}"
                           class="w-full rounded-2xl border border-slate-300 p-3 focus:border-blue-500 focus:ring-blue-100"
                           required>
                </div>
            ...
            <div class="space-y-4">
                ...
                <textarea name="description"
                          class="min-h-[120px] w-full rounded-3xl border border-slate-300 p-4 focus:border-blue-500 focus:ring-blue-100">{{ old('description', $property->description) }}</textarea>
            </div>

            <div class="space-y-4">
                <label class="block text-sm font-medium text-slate-700">About This Property</label>
                <textarea name="about"
                          class="min-h-[140px] w-full rounded-3xl border border-slate-300 p-4 focus:border-blue-500 focus:ring-blue-100">{{ old('about', $property->about) }}</textarea>
            </div>
            ...
            <div class="flex justify-end">
                <button type="submit"
                        class="inline-flex items-center justify-center rounded-2xl bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow hover:bg-blue-700">
                    Update Property
                </button>
            </div>
        </form>

        {{-- ✅ IMAGE SECTION (OUTSIDE FORM FIX) --}}
                ...
                <div class="grid grid-cols-3 gap-3">
                    @foreach($property->images as $img)
                        <div class="relative overflow-hidden rounded-3xl">
                            ...
                            <form method="POST"
                                  action="{{ route('admin.properties.image.delete', $img) }}"
                                ...
                                <button type="submit"
                                        ...
                                        class="rounded-full bg-white/90 px-2 py-1 text-xs text-red-600 shadow-sm">
                                    ✕
                                </button>
                            </form>
                        </div>
